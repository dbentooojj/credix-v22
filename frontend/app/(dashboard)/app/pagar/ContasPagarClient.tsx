...
export function ContasPagarClient() {
  ...
  return (
    <div className="w-full max-w-[1600px] mx-auto pb-24 lg:pb-8">
      ...
      <PageHeader
        subtitle="Acompanhe e organize os compromissos financeiros de saída."
      ...
      >
        <div className="grid grid-cols-2 gap-4">
          ...
          {!isEditing && formCreationMode === "installments" && (
            <>
              <ModalField label="Quantidade de parcelas">
                <input className={modalInputClass} type="number" min="2" max="60" value={formInstallmentCount} onChange={(e) => { setFormInstallmentCount(e.target.value); clearCustomInstallmentDueDates(); }} />
              </ModalField>
          ...
          <ModalField label="Data de vencimento">
            <input className={modalInputClass} type="date" value={formDate} onChange={(e) => { setFormDate(e.target.value); if (!isEditing && formCreationMode === "installments") { clearCustomInstallmentDueDates(); } }} />
          </ModalField>
                    ...
                    installmentPlanRows.map((item) => (
                      <div key={item.installmentNumber} className="rounded-lg border border-slate-700/70 bg-slate-900/70 p-3">
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <p className="text-sm font-bold text-slate-100">{formatCurrency(item.amount)}</p>
                        </div>
