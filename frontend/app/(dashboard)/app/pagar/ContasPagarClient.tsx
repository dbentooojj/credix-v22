    ...
    <div className="w-full max-w-[1600px] mx-auto pb-24 lg:pb-8">
      ...
      <section className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1.5 text-sm text-slate-400">Gerencie despesas, vencimentos e pagamentos do seu negócio.</p>
        </div>
                    ...
                    <tr key={item.id} className="transition-colors hover:bg-slate-800/40">
                      ...
                      <td className="px-4 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button onClick={() => openViewModal(item)} className="flex h-8 w-8 items-center justify-center rounded-lg border border-blue-500/30 bg-blue-500/10 text-blue-400 transition-colors hover:bg-blue-500/20" title="Ver detalhes">
                            <Eye className="h-4 w-4" />
                          </button>
                          {ds.group !== "paid" && (
                            <button onClick={() => handleComplete(item)} className="flex h-8 items-center justify-center gap-1 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-2 text-xs font-semibold text-emerald-400 transition-colors hover:bg-emerald-500/20" title="Marcar como paga">
                              <CheckCircle2 className="h-3.5 w-3.5" />
                              Pagar
                            </button>
                          )}
                          <button onClick={() => openEditModal(item)} className="flex h-8 w-8 items-center justify-center rounded-lg border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 transition-colors hover:bg-emerald-500/20" title="Editar">
                            <Edit2 className="h-4 w-4" />
                          </button>
                          <button onClick={() => openDeleteModal(item)} className="flex h-8 w-8 items-center justify-center rounded-lg border border-red-500/30 bg-red-500/10 text-red-400 transition-colors hover:bg-red-500/20" title="Excluir">
      ...
      >
        <div className="grid grid-cols-2 gap-4">
          <ModalField label="Descrição" full>
            <input className={modalInputClass} maxLength={300} placeholder="Ex: Aluguel do escritório" value={formDescription} onChange={(e) => setFormDescription(e.target.value)} />
          </ModalField>
          <ModalField label="Categoria">
            <input className={modalInputClass} maxLength={120} placeholder="Ex: Aluguel" value={formCategory} onChange={(e) => setFormCategory(e.target.value)} />
          </ModalField>
      ...
      <ModalBase
        ...
        subtitle="Visualize as informações desta conta."
        footer={
          <>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
                  ...
                  <p className="mt-1 text-sm font-semibold text-slate-200">{viewingItem.category || "—"}</p>
                </div>
                <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
                ...
                <div className="rounded-xl border border-slate-700/50 bg-slate-800/40 p-3">
                  <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Observação</p>
